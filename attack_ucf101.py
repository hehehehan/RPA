...
def arg_parse():
    ...
    parser.add_argument('--attack_method', type=str, default='RPA', help='FGSM | RPA')
    ...
    parser.add_argument('--iterative_momentum', action='store_true', default=True, help='Use iterative momentum in MFFGSM.')
...
if __name__ == '__main__':
    ...
    if args.attack_type == 'image':
        ...
        if args.attack_method == 'RPA':
            spe_params = {'kernlen':15, 'momentum':True, 'weight':1.0, 'move_type':'adj', 'kernel_mode':'gaussian'}
        print ('Used Params')
        print (spe_params)
        attack_method = getattr(RPA_video_attacks, args.attack_method)(model, params=spe_params, steps=args.step)
